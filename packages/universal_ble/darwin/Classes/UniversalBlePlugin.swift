...
private class BleCentralDarwin: NSObject, UniversalBlePlatformChannel, CBCentralManagerDelegate, CBPeripheralDelegate {
  ...
  // Deferred scan: on macOS 26, calling scanForPeripherals when manager.state
  // is .unknown causes a SIGTERM.  Store params and start once poweredOn.
  ...
  func getBluetoothAvailabilityState(completion: @escaping (Result<Int64, Error>) -> Void) {
    ...
    // Only block when explicitly denied/restricted.
    // On macOS 14+, .notDetermined means "never asked" but BLE works for
    // non-App-Store apps without explicit permission.
  ...
  func requestPermissions(withAndroidFineLocation _: Bool, completion: @escaping (Result<Void, any Error>) -> Void) {
    ...
    switch auth {
    case .denied:
      completion(.failure(createFlutterError(code: .bluetoothUnauthorized,
        message: "Bluetooth access denied. Enable Wattalizer in System Settings > Privacy & Security > Bluetooth.")))
    case .restricted:
      completion(.failure(createFlutterError(code: .notSupported,
        message: "Bluetooth access is restricted on this device.")))
    default:
      // .allowedAlways or .notDetermined — on macOS 14+, .notDetermined is fine
      // for non-App-Store apps; CBCentralManager works without explicit permission.
      if manager.state != .unknown {
        completePermissionRequest(completion: completion)
      } else {
        // Trigger lazy init (registers BT intent, starts async state resolution)
        // then complete immediately.  On macOS 26, waiting for
        // centralManagerDidUpdateState causes Flutter to kill the process (~2s
        // platform-message reply timeout).  Any subsequent scanForPeripherals
        // call is deferred in startScan until centralManagerDidUpdateState fires
        // with .poweredOn (calling it while state is .unknown triggers SIGTERM
        // on macOS 26).
        // On macOS 26 (Tahoe), TCC crashes the app with
        // __TCC_CRASHING_DUE_TO_PRIVACY_VIOLATION__ if CBCentralManager.init is
        // called while the app is not the active foreground application.
        // NSApp.activate is async — we must wait for didBecomeActiveNotification
        // before touching CBCentralManager, otherwise the XPC request to tccd
        // arrives while the app is still inactive and TCC kills us.
        #if os(OSX)
        if !NSApp.isActive {
  ...
  func startScan(filter: UniversalScanFilter?, config _: UniversalScanConfig?) throws {
    ...
    if manager.state == .poweredOn {
      ...
      // macOS 26: calling scanForPeripherals when state is .unknown triggers
      // SIGTERM.  Defer until centralManagerDidUpdateState fires with poweredOn.
  ...
  func centralManagerDidUpdateState(_ central: CBCentralManager) {
    // Start any deferred scan now that state is known.
    ...
    // Capture handlers before dispatching — mutations must stay on bleQueue.
    let availHandlers = availabilityStateUpdateHandlers
    let permHandlers = requestPermissionStateUpdateHandlers
    availabilityStateUpdateHandlers.removeAll()
    requestPermissionStateUpdateHandlers.removeAll()
    // Dispatch Flutter callbacks to main thread — FlutterBinaryMessenger and
    // Pigeon reply closures must be called from the main thread on macOS.
    DispatchQueue.main.async { [weak self] in
      guard let self = self else { return }
      self.callbackChannel.onAvailabilityChanged(state: state) { _ in }
      for handler in availHandlers {
        handler(.success(state))
      }
      for handler in permHandlers {
        self.completePermissionRequest(completion: handler)
      }
    }
  }
